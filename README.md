...
## 게시글
**현재 게시글 기준 이전/다음 게시글 네비게이션** 추가


**이미지업로드** : media/posts/유저아이디/게시글고유번호/이미지명
- 1단계 : 파일명 중복 방지(UUID), 파일 시스템 정리(Django-cleanup)
- 2단계(예정) : 파일 크기 제한(5MB), 게시글 당 5개 제한, 파일 형식 제한(jpg, png, webp) 
- 3단계(예정) : 이미지 자동 리사이징(프로필:500x500이하, 포스트1920x1080이하), 썸네일 생성, 지연 로딩
- 4단계(예정) : 보안 강화(악의적 대용량 파일 업로드 차단, 서버 과부하 공격 방지)
